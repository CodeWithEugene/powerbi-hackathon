...
│   └── TEAM_PLAYBOOK.md                            # Findings, cleaning logic, DAX, storyboard, pitch — read first
├── 1.data/
│   ├── raw/                                        # Source data (from https://smplu.link/BIHACKATHON) — never edit
...
│   └── cleaned/                                    # Generated by 2.models/clean_data.py
│       ├── clean_customers.csv / clean_activity.csv / clean_calendar.csv   # Power BI star schema
│       ├── AnganiAir_Model.xlsx                    # Same 3 tables for Power BI Service upload
│       └── clean_flat_activity_customers.csv       # Single-table fallback
├── 2.models/                                       # Python: cleaning, validation, analysis
│   ├── clean_data.py                               # Source CSVs → clean_* files
│   ├── validate.py                                 # Control-total checks (exit 1 on failure)
│   └── analyze.py                                  # Reproduces every finding in the playbook
...
2. Run `python3 2.models/clean_data.py && python3 2.models/validate.py` to generate and check the clean tables. Cleaning rules are explained in `0.docs/TEAM_PLAYBOOK.md` §3.
